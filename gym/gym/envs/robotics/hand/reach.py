...
class HandReachEnv(hand_env.HandEnv, utils.EzPickle):
    ...
    def _get_achieved_goal(self):
        # site coords for sites on the finger tip
    ...
    def _render_callback(self):
        ...
        # floor0 absolute coord = site absolute coord - site relative coord (to floor0)
        # this can be read from self.sim.data.xpos[1] -- which is floor0
        sites_offset = (self.sim.data.site_xpos - self.sim.model.site_pos).copy()
        # the following is equivalent
        # we only index 10 lines!!!
        #sites_offset = np.array([self.sim.data.body_xpos[1]] * 10)

        # print geometric center vs. center of mass
        #print('xpos',self.sim.data.site_xpos)
        #print('pos',self.sim.model.site_pos)
        ...
        for finger_idx in range(5):
            ...
            # site coord = goal relative coord = goal abs coord - floor0 abs coord
            # site coord defined in reach.xml
        ...
        for finger_idx in range(5):
            ...
            site_id = self.sim.model.site_name2id(site_name)
            # site coord = finger relative coord = finger abs coord - floor0 abs coord
            # site coord defined in reach.xml
