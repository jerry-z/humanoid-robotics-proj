...
class GraspRelativeSafeThreeEnv(simple_robotics_env.SimpleRoboticsEnv, utils.EzPickle):
    ...
    def reset_model(self):
        ...
        while failed:
            ...
            Ssize = np.array([0.015, 0.015, 0.1])
            Msize = np.array([0.025, 0.025, 0.1])
            Lsize = np.array([0.025, 0.025, 0.1])
            rndindex = np.random.randint(3)
            if rndindex == 0:
                self.sim.model.geom_size[-2, :] = Ssize
            if rndindex == 1:
                self.sim.model.geom_size[-2, :] = Msize
            if rndindex == 2:
                self.sim.model.geom_size[-2, :] = Lsize
...
class GraspCubeRelativeSafeThreeEnv(GraspRelativeSafeThreeEnv):
    def __init__(self):
        GraspRelativeSafeThreeEnv.__init__(self, 'seahand/manipulate_block.xml', 1)
