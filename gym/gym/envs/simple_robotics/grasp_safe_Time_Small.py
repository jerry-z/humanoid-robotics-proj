...
class GraspRelativeSafeTimeSmallEnv(simple_robotics_env.SimpleRoboticsEnv, utils.EzPickle):
    ...
    def reset_model(self):
        ...
        while failed:
            ...
            LOW_limit = np.array([0.01, 0.01, 0.1])
            HIGH_limit = np.array([0.02, 0.02, 0.1])
            LOW_start = np.array([0.03, 0.03, 0.1])
            HIGH_start = np.array([0.04, 0.04, 0.1])
            reset_epi =  self.reset_time // 1000
            LOW = LOW_start - np.array([reset_epi * 0.001, reset_epi * 0.001, 0])
            HIGH = HIGH_start - np.array([reset_epi * 0.001, reset_epi * 0.001, 0]) 
            LOW = np.clip(LOW, LOW_limit, np.inf)
            HIGH = np.clip(HIGH, HIGH_limit, np.inf)
...
class GraspCubeRelativeSafeTimeSmallEnv(GraspRelativeSafeTimeSmallEnv):
    def __init__(self):
        GraspRelativeSafeTimeSmallEnv.__init__(self, 'seahand/manipulate_block.xml', 1)


class GraspEggRelativeSafeTimeSmallEnv(GraspRelativeSafeTimeSmallEnv):
    def __init__(self):
        GraspRelativeSafeTimeSmallEnv.__init__(self, 'seahand/manipulate_egg.xml', 1)


class GraspPenRelativeSafeTimeSmallEnv(GraspRelativeSafeTimeSmallEnv):
    def __init__(self):
        GraspRelativeSafeTimeSmallEnv.__init__(self, 'seahand/manipulate_pen.xml', 1)


class GraspBallRelativeSafeTimeSmallEnv(GraspRelativeSafeTimeSmallEnv):
    def __init__(self):
        GraspRelativeSafeTimeSmallEnv.__init__(self, 'seahand/manipulate_ball.xml', 1)


class GraspCylinderRelativeSafeTimeSmallEnv(GraspRelativeSafeTimeSmallEnv):
    def __init__(self):
        GraspRelativeSafeTimeSmallEnv.__init__(self, 'seahand/manipulate_cylinder.xml', 1)
