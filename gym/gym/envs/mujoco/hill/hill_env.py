...
MODEL_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), 'mujoco_models'))
FIELD_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), 'fielddata'))
# print (MODEL_DIR)

# class HillEnv(gym.Wrapper, Serializable):
class HillEnv(gym.Wrapper, utils.EzPickle):
    ...
    def __init__(self,
                 difficulty=10.0,
                 texturedir=FIELD_DIR + '/mujoco_textures_xup',#'/tmp/mujoco_textures',
                 hfield_dir=FIELD_DIR + '/mujoco_terrains_xup',#'/tmp/mujoco_terrains',
                 regen_terrain=False,
                 *args,
                 **kwargs):
        ...
        print ("understand what is self.__class__.MODEL_CLASS:", model_cls, self.__class__)
        if not model_cls:
            raise NotImplementedError("MODEL_CLASS unspecified!")

        template_file_name = 'hill_' + model_cls.__module__.split(
            '.')[-1] + '.xml.mako'
        print ("understand template_file_name", template_file_name)
        template_options = dict(
            difficulty=self.difficulty,
            texturedir=self.texturedir,
            hfield_file=os.path.join(self.hfield_dir, self.HFIELD_FNAME),
            texture_file=os.path.join(self.texturedir, self.TEXTURE_FNAME))
        print ("understand hfield file:", os.path.join(self.hfield_dir, self.HFIELD_FNAME))
        file_path = os.path.join(MODEL_DIR, template_file_name)
        print ("understand file_path:", file_path)
    ...
    def _gen_terrain(self, regen=True):
        ...
        x, y, hfield = terrain.generate_x_uphill(40, 40, 1.0)
