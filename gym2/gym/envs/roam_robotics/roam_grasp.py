...
from gym.envs.roam_robotics import roam_simple_robotics_env
from gym.envs.roam_robotics import rotations
...
class ROAMGraspEnv(roam_simple_robotics_env.ROAMSimpleRoboticsEnv, utils.EzPickle):
    ...
    def step(self, action, render=False):
        ...
        self._set_action(action) #RECENT COMMENTE
        self.sim.step()
        obs = self._get_obs()
    ...
    def _get_obs(self):
        handpos = self.sim.data.sensordata[:12]
        ...
        print("state: ", np.shape(state), state)
        return state
    ...
    def _set_action(self, action):
        ...
        print("Asdfasdf",np.shape(actuation_range))
        # relative command   
        #relative_cmd = actuation_center + action * actuation_range
        #print('relative cmd',relative_cmd)
        #if self.prev_cmd is None:
        #    self.prev_cmd = np.zeros_like(relative_cmd)
        self.sim.data.ctrl[:] = actuation_center + action * actuation_range
        self.sim.data.ctrl[:] = np.clip(self.sim.data.ctrl, ctrlrange[:, 0], ctrlrange[:, 1])
        print('sim ctrl',self.sim.data.ctrl[:])
    ...
    def reset_model(self):
        ...
        while failed:
            ...
            #print("Qve", np.shape(qvel), qvel)
            print("QPos", np.shape(qpos), qpos)

            # set state
            self.set_state(qpos, qvel)
            failed = False #FIX THIS!!! 
    ...
    def compute_torques(self):
        torques = self.sim.data.sensordata[12:]
        assert torques.size == 24
        print("shaaape", np.shape(torques))
        print('torques', torques)
        torques = np.array([np.linalg.norm(torques[:3]), np.linalg.norm(torques[3:6]), np.linalg.norm(torques[6:9]), np.linalg.norm(torques[9:12]), np.linalg.norm(torques[12:15]), np.linalg.norm(torques[15:18]), np.linalg.norm(torques[18:21]), np.linalg.norm(torques[21:24])])
    ...
    def compute_reward(self, version='naive'):
        ...
        if version is 'naive':
            ...
            #print(self.distal_contact(), self.fingers_contact())
        ...
        """
        if not self.object_floor_contact():
            return 1.0
        else:
            return 0.0
    ...
    def object_floor_contact(self):
        # print ("check floor id is 0:", self.model.geom_name2id('floor0'))
        floorid = self.model.geom_name2id('floor0')
        objectid = self.model.geom_name2id('object')
        ncon = self.sim.data.ncon
        contacts = self.sim.data.contact[:ncon]
        for contact in contacts:
    ...
    def in_reach(self): #NEEDS TO BE REWRITTEN
        objpos = self.sim.data.get_body_xpos("object")
        palmpos = self.sim.data.get_body_xpos("robot0:base_link_palm")
        center = np.zeros(3)
