...
TORQUE_LIMIT = 0.6 # termination condition for torques

class GraspRelativeSafeRenderEnv(simple_robotics_env.SimpleRoboticsEnv, utils.EzPickle):
    ...
    def reset_model(self):
        ...
        while failed:
            ...
            assert self.anglerange is not None
            angle = self.np_random.uniform(-np.pi, np.pi) / (180 / self.anglerange) # self.anglerange is deg
            axis = np.array([0., 0., 1.])
...
class GraspCubeRelativeSafeRenderEnv(GraspRelativeSafeRenderEnv):
    def __init__(self):
        GraspRelativeSafeRenderEnv.__init__(self, 'seahand/manipulate_block.xml', 1)
