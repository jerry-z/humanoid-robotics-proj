...
class GraspRelativeSmoothEnv(simple_robotics_env.SimpleRoboticsEnv, utils.EzPickle):
    ...
    def compute_reward(self, version='naive'):
        ...
        sum_torques = self.shape_torques()
        if version is 'naive':
            # print ("naive")
            return sum_torques
    ...
    def shape_torques(self):
        torques = self.compute_torques()
        if (torques > 0.6).any():
            return 0
        if (torques > 0.35).any():
           return sum(np.array([0.84, 0.42, 0.84, 0.42]) - torques * 1.4) # 0.84 - 1.4 * torque = reward, linear interpolation
        return sum(torques)
...
class GraspCubeSmoothEnv(GraspRelativeSmoothEnv):
    def __init__(self):
        GraspRelativeSmoothEnv.__init__(self, 'seahand/manipulate_block.xml', 1)
