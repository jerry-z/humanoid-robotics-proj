...
class GraspLiftEnv(simple_robotics_env.SimpleRoboticsEnv, utils.EzPickle):
    ...
    def step(self, action, render=False):
        ...
        if not render:
            # print ("show if in reach:", self.in_reach())
            # print ("show if safe:", self.safe_torques())
    ...
    def _set_action(self, action):
        ...
        ctrlrange = self.sim.model.actuator_ctrlrange[:-1]
        # action transform
        ...
        # clip
        self.sim.data.ctrl[:-1] = new_cmd
        self.sim.data.ctrl[:-1] = np.clip(self.sim.data.ctrl[:-1], ctrlrange[:, 0], ctrlrange[:, 1])
        self.prev_cmd = self.sim.data.ctrl[:-1].copy()
    ...
    def reset_model(self):
        ...
        while failed:
            ...
            LOW = np.array([0.01, 0.01, 0.07])
            HIGH = np.array([0.04, 0.04, 0.07])
            ...
            # random object com
            print ("init object position:", self.init_objpos)
            initial_obj_com = self.init_objpos[1:4] + self.np_random.uniform(size=3, low=-.01, high=.01)
            # random object z rotation
            angle = self.np_random.uniform(-np.pi, np.pi)
            axis = np.array([0., 0., 1.])
            offset_quat = rotations.quat_from_angle_and_axis(angle, axis)
            initial_obj_quat = self.init_objpos[4:].copy()
            initial_obj_quat = rotations.quat_mul(initial_obj_quat, offset_quat)
            initial_obj_quat /= np.linalg.norm(initial_obj_quat)
            qpos = np.concatenate((self.init_handpos,np.array([0]), initial_obj_com, initial_obj_quat))
            # set state
...
class GraspCubeLiftEnv(GraspLiftEnv):
    def __init__(self):
        GraspLiftEnv.__init__(self, 'seahand/lift_block.xml', 1)


class ROAM_GraspCubeLiftEnv(GraspLiftEnv):
    def __init__(self):
        GraspLiftEnv.__init__(self, 'seahand/lift_block.xml', 1)
